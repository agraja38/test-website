...
export function normalizeFeed(slug: AppSlug, feed: GenericFeed): FeedState {
  ...
  if (slug === "justquit") {
    ...
    return {
      ...
      releaseNotesURL,
      downloads: [
        ...(dmgURL ? [option("macOS installer", "macOS", dmgURL, undefined, undefined, true)] : []),
        ...(zipURL ? [option("In-app update ZIP", "macOS", zipURL, undefined, asNumber(feed.sizeBytes))] : []),
      ],
    };
  }

  if (slug === "justquit-windows" || slug === "winswitch") {
    const x64URL = asString(feed.downloadUrl);
    const armURL =
      slug === "winswitch"
        ? x64URL?.replace("WinSwitch-Setup-x64.exe", "WinSwitch-Setup-ARM64.exe")
        : x64URL?.replace("justQuit-Setup-x64.exe", "justQuit-Setup-ARM64.exe");
